  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "bg-white/90 backdrop-blur-xl shadow-xl shadow-[#800000]/5"
          : "bg-transparent"
      }`}
    >
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span className="text-xl tracking-tight">
              <span className="font-bold bg-gradient-to-r from-[#800000] to-[#A64253] bg-clip-text text-transparent">
                Data Darling
              </span>
            </span>
          </button>
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <button
                ...
                className="relative px-5 py-2 text-[#2D1B1B] hover:text-[#800000] transition-colors group"
              >
                {link.label}
          ...
          <button
            ...
            className="md:hidden p-2 rounded-xl bg-gradient-to-br from-[#800000] to-[#A64253] text-[#F2E8C6] hover:shadow-lg transition-all"
          >
          ...
          <div className="md:hidden mt-4 py-4 space-y-2 bg-white/95 backdrop-blur-xl rounded-2xl px-4 shadow-xl border border-[#800000]/10">
            {navLinks.map((link) => (
              <button
                key={link.id}
                onClick={() => scrollToSection(link.id)}
                className="block w-full text-left text-[#2D1B1B] hover:text-[#800000] hover:bg-[#F2E8C6]/50 transition-all py-3 px-4 rounded-xl"
              >
                {link.label}
              </button>
            ))}
